 ...
 *
 * Text-only workflow (no image/video inputs): no S3 infrastructure needed.
 * Fixture text is read directly from the colocated prompt_location.txt file.
 *
 * Run inside the harness container (standard fat-image deployment):
 *   docker exec -it harness golem repl \
 *     --script-file /work/workflows/krea2_location_sheet/e2e-test.ts --yes
 *
 ...
 *   golem repl --script-file ../workflows/krea2_location_sheet/e2e-test.ts --yes
 */
...
const PROVISION_TIMEOUT_MS = 2400000;  // ~40 min — large model download (~50 GB total)
const RENDER_TIMEOUT_MS    = 3600000;  // ~60 min — KREA-2 + H3 pipeline
...
try {
  ...
  for (const model of (parsedYaml.models ?? [])) {
    for (const f of (model.files ?? [])) {
      ...
      if (url.includes('civitai.com') && CIVITAI_TOKEN) {
        probeUrl = url + (url.includes('?') ? '&' : '?') + `token=${CIVITAI_TOKEN}`;
      }
      const resp = await fetch(probeUrl, { method: 'HEAD', redirect: 'follow', headers });
  ...
  // 3. Read fixture text ─────────────────────────────────────────────────────
  console.log('\n── 3. Load fixture text ─────────────────────────────────────────');
  const promptLocationText = _nodeFs.readFileSync(
    _path.join(FIXTURE_DIR, 'prompt_location.txt'), 'utf-8'
  ).toString().trim();
  check('prompt_location fixture loaded', promptLocationText.length > 0, `${promptLocationText.length} chars`);
  console.log(`  prompt_location: "${promptLocationText.slice(0, 80)}${promptLocationText.length > 80 ? '…' : ''}"`);
  ...
  const renderId = await agent.run(
    ...
    JSON.stringify({}),
    JSON.stringify({
      prompt_location: promptLocationText,
      width: 1920,
      height: 1080,
      seed: 42,
      steps: 8,
  ...
  if (result.phase === 'succeeded' && result.outputs?.length > 0) {
    console.log('\n── 6. Verify outputs ────────────────────────────────────────────');
    // Expect all 9 declared ports: location_sheet (primary), orbit_video, anchor_preview,
    // view_1..view_6 (individual pose-select frames split out of node 60's batch)
    check('all 9 outputs returned', result.outputs.length === 9, `got ${result.outputs.length}`);
    for (const out of result.outputs) {
      ...
      if (out.kind === 'video') {
        ...
        check(`${out.name ?? out.filename} is PNG`, _isPng(raw), `magic=${hex}`);
      }
    }
  } else {
